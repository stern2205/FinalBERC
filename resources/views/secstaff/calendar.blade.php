...
<div class="max-w-7xl mx-auto px-4 py-4">
        ...
        <div id="tour-calendar-legend" class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <div class="flex items-center gap-3">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <span class="legend-item"><span class="legend-box" style="background-color: #0f766e;"></span>At Chair</span>
                <span class="legend-item"><span class="legend-box" style="background-color: #b45309;"></span>At Applicant</span>
        ...
        <div id="tour-calendar-rules" class="flex flex-wrap items-center gap-4 mb-3 px-4 py-2.5 bg-white border border-[#dde5f5] rounded-xl shadow-sm text-[11px] font-semibold text-gray-600">
        ...
        <div id="tour-calendar-filters" class="flex flex-wrap items-center gap-2 mb-3">
            ...
            <button class="filter-btn" data-filter="chair"><span style="color:#0f766e">●</span> At Chair</button>
            <button class="filter-btn" data-filter="applicant"><span style="color:#b45309">●</span> At Applicant</button>
        ...
        <div class="flex flex-col lg:flex-row gap-4">
            ...
            <div id="tour-calendar-grid" class="flex-1 min-w-0 overflow-hidden">
                <div class="cal-outer">
            ...
            <div id="tour-calendar-sidebar" class="lg:w-72 xl:w-80 shrink-0">
                <div class="mb-3 bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center justify-between">
                            <span class="flex items-center gap-2 text-[11px] font-semibold text-gray-600">
                                <span class="w-2.5 h-2.5 rounded-sm bg-[#0f766e] inline-block"></span>At Chair
                            </span>
                            <span class="text-[13px] font-black text-teal-800" id="statChair">—</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="flex items-center gap-2 text-[11px] font-semibold text-gray-600">
                                <span class="w-2.5 h-2.5 rounded-sm bg-[#b45309] inline-block"></span>At Applicant
                            </span>
                            <span class="text-[13px] font-black text-orange-800" id="statApplicant">—</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="flex items-center gap-2 text-[11px] font-semibold text-gray-600">
...
<script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Trigger ONLY if they arrived from the Applications page
            if (tourState === 'sec_calendar') {

                if (typeof window.driver === 'undefined') {
                    console.error("Driver.js is missing on the Calendar page!");
                    return;
                }
                ...
                const tour = driver({
                    ...
                    onDestroyStarted: () => {
                        if (!tour.hasNextStep()) {
                            // Move to the Application History page next
                            localStorage.setItem(storageKey, 'sec_history');
                            tour.destroy();
                            window.location.href = "{{ route('secstaff.history') }}";
                        } else {
                            tour.destroy();
                        }
                    },
                    ...
                    steps: [
                        {
                            ...
                            popover: {
                                ...
                                description: 'Every protocol is color-coded based on its current status. This allows you to instantly see who is currently holding up the review process.',
                                side: "bottom",
                                align: 'start'
                            }
                        },
                        {
                            ...
                            popover: {
                                title: 'Key Dates',
                                description: 'Committee meeting days and application cut-off dates are automatically generated and marked on the calendar for you.',
                                side: "bottom",
                                align: 'start'
                            }
                        },
                        {
                            ...
                            popover: {
                                title: 'Filter Views',
                                description: 'Too much clutter? Use these buttons to isolate specific statuses—like filtering to show ONLY the applications currently waiting at your desk.',
                                side: "bottom",
                                align: 'start'
                            }
                        },
                        ...
                        },
                        {
                            ...
                            popover: {
                                ...
                                description: 'A quick breakdown of the month\'s workload. Scroll down here to see an organized list of all upcoming tasks for the current month.',
                                side: "left",
                                align: 'start'
                            }
                        },
                        {
                            // Floating popover
                            popover: {
                                title: 'Next Stop: History Hub',
                                description: 'Once applications are finalized, they leave the calendar and go to the archives. Let\'s look at the History page next.',
                                side: "bottom",
                                align: 'center',
                                doneBtnText: 'Next Page →' // Sends them to secstaff.history
                            }
                        }
                    ]
                });
        ...
        // ── Type & Location Helpers (UPDATED FOR CHAIR AND APPLICANT) ──
        function typeColor(type) {
            return {
                meeting:'#f97316', deadline:'#dc2626', secretariat:'#1e40af',
                reviewer:'#92400e', assigned:'#166534', cutoff:'#7c3aed',
                chair: '#0f766e', applicant: '#b45309'
            }[type] || '#6b7280';
        }
        function typeLabel(type) {
            return {
                meeting:'Meeting', deadline:'Deadline', secretariat:'At Secretariat',
                reviewer:'At Reviewer', assigned:'Assigned', cutoff:'Cut-off',
                chair: 'At Chair', applicant: 'At Applicant'
            }[type] || 'Event';
        }
        function locationLabel(loc) {
            return {
                secretariat:'Secretariat', reviewer:'Reviewer', assigned:'Awaiting Acceptance',
                chair:'Chair', applicant:'Applicant (Revision)', completed:'Completed'
            }[loc] || '—';
        }
        function typePillClass(type) {
            return {
                meeting:'loc-pill-meet', deadline:'loc-pill-dead', secretariat:'loc-pill-sec',
                reviewer:'loc-pill-rev', assigned:'loc-pill-asgn', cutoff:'loc-pill-cut',
                chair: 'bg-teal-100 text-teal-800', applicant: 'bg-orange-100 text-orange-800'
            }[type] || 'loc-pill-sec';
        }
        function typeDateBoxClass(type) {
            return {
                meeting:'meeting', deadline:'deadline', secretariat:'',
                reviewer:'reviewer', assigned:'assigned', cutoff:'cutoff',
                chair:'chair', applicant:'applicant'
            }[type] || '';
        }
        function typeChipClass(type) {
            return {
                meeting:'ev-chip-meeting', deadline:'ev-chip-deadline', secretariat:'ev-chip-secretariat',
                reviewer:'ev-chip-reviewer', assigned:'ev-chip-assigned', cutoff:'ev-chip-cutoff',
                chair: 'text-teal-700 bg-teal-50 border-teal-200', applicant: 'text-orange-700 bg-orange-50 border-orange-200'
            }[type] || 'ev-chip-secretariat';
        }
        ...
        async function fetchDynamicEvents(year, month) {
            ...
            try {
                ...
                const fetchUrl = "{{ url('/calendar/events') }}?year=" + year + "&month=" + apiMonth;
        ...
        function makeEventCard(day, ev, isForModal = false) {
                    ...
                    <div class="hidden expandable-content border-t border-gray-100 bg-[#FAFAFA] p-5">
                        ...
                        ${ev.appId ? `
                            <div class="mt-6 text-right">
                                <a href="/secstaff/history?openApp=${ev.appId}" class="inline-flex items-center justify-center px-5 py-2.5 bg-brand-red text-white text-[10px] font-bold uppercase tracking-widest rounded-lg hover:bg-red-700 transition-colors shadow-sm">
                                    View Full Application
                                    <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" class="ml-2">
            ...
            // ==========================================
                ...
                wrapper.className = 'side-event-item' + (ev.appId ? ' side-event-clickable block hover:bg-gray-50 transition-colors' : '');
                if (ev.appId) wrapper.setAttribute('href', `/secstaff/history?openApp=${ev.appId}`);
        ...
        // ── Month Sidebar ──
        // ── Month Sidebar (STRICTLY TODAY ONLY) ──
            ...
            for (let i = 0; i < totalCells; i++) {
                ...
                if (isCurrentMonth) {
                    ...
                    cell.addEventListener('click', () => {
                        if (visEvs.length > 0 || isMeeting || isCutoff) {
    ...
    const tour = driver({
        ...
        steps: [
            ...
            },
            {
                ...
                popover: {
                    title: 'Monthly Summary',
                    description: 'A quick breakdown of the month’s workload. Scroll here to see an organized list of upcoming tasks.',
                    side: "left",
                    align: 'start'
                }
            },
            {
                popover: {
                    title: 'Next Stop: History Hub',
                    description: 'Once applications are finalized, they move to the archives. Let’s continue to the History page.',
                    side: "bottom",
                    align: 'center',
