...
<head>
    ...
    <style>
        body {
            ...
            margin: 0;
            padding: 0;
            /* This kills the height of invisible spaces */
            font-size: 0;
        }

        /* This restores the normal font size for your actual layout */
        body > * {
            font-size: 16px;
        }
...
<body class="bg-gray-50">
    <header class="bg-white sticky top-0 z-50 shadow-sm" x-data="{ isNavDrawerOpen: false, isProtocolRoute: {{ request()->routeIs('pipeline.*') ? 'true' : 'false' }}, protocolMenuOpen: {{ request()->routeIs('pipeline.*') ? 'true' : 'false' }}, mobileProtocolOpen: false }">
        ...
        <div class="border-t border-gray-100 bg-white hidden md:block">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between text-[11px] font-bold uppercase tracking-wider text-gray-500">
                ...
                <div class="flex space-x-1">
                    ...
                    <a href="{{ route('reviewer.calendar') }}" x-show="!isProtocolRoute" x-cloak @click="protocolMenuOpen = false"
                    class="flex items-center gap-2 px-5 py-3.5 text-[11px] font-bold uppercase tracking-widest transition border-b-[3px]
                    ...
                    <div class="flex items-stretch">
                        <button type="button" x-show="!protocolMenuOpen" x-cloak @click="protocolMenuOpen = true"
                                class="flex items-center gap-2 px-5 py-3.5 text-[11px] font-bold uppercase tracking-widest transition border-b-[3px]
                                {{ request()->routeIs('pipeline.*') ? 'text-bsu-dark border-brand-red bg-white' : 'text-gray-500 border-transparent hover:text-bsu-dark hover:border-brand-red' }}">
                            <svg class="w-4 h-4 {{ request()->routeIs('pipeline.*') ? 'text-bsu-dark' : 'text-brand-red' }} shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div x-show="protocolMenuOpen" x-cloak
                            ...
                            <a href="{{ route('reviewer.invitations') }}" class="flex items-center px-5 py-3.5 whitespace-nowrap transition border-b-[3px] border-transparent text-gray-500 hover:text-bsu-dark hover:border-brand-red hover:bg-gray-50">
        ...
        <div x-show="isNavDrawerOpen" x-collapse x-cloak class="md:hidden border-t border-gray-100 bg-white shadow-inner">
            <div class="flex flex-col text-[11px] font-bold uppercase tracking-wider text-gray-500 divide-y divide-gray-100">
                ...
                <a href="{{ route('reviewer.calendar') }}" class="flex items-center space-x-3 px-5 py-4 transition-colors {{ request()->routeIs('calendar') ? 'text-bsu-dark bg-gray-50 border-l-4 border-brand-red' : 'hover:bg-gray-50 hover:text-bsu-dark' }}">
                    <span>Calendar</span>
                </a>

                <button type="button" @click="mobileProtocolOpen = !mobileProtocolOpen" class="w-full flex items-center justify-between px-5 py-4 focus:outline-none transition-colors {{ request()->routeIs('pipeline.*') ? 'text-bsu-dark bg-gray-50 border-l-4 border-brand-red' : 'text-gray-500 hover:bg-gray-50 hover:text-bsu-dark' }}">
                    <span>MY APPLICATIONS</span>
                ...
                <div x-show="mobileProtocolOpen" x-collapse x-cloak class="bg-gray-50 border-t border-b border-gray-100">
                    <a href="{{ route('reviewer.invitations') }}" @click="isNavDrawerOpen = false; mobileProtocolOpen = false" class="block pl-9 pr-5 py-3 text-[10px] font-bold transition-colors text-gray-500 hover:text-bsu-dark hover:bg-gray-100">
                        Invitation
                    </a>
