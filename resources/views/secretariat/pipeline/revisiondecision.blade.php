...
<style>
    ...
    .assessment-grid-header, .assessment-row { display: grid; grid-template-columns: minmax(140px, 1fr) minmax(300px, 2.2fr) minmax(180px, 1.3fr) minmax(180px, 1.2fr) minmax(140px, 1fr); padding: 14px 20px; align-items: center; gap: 12px; }
...
<div x-data="decisionData(@js($protocolsData ?? []))" class="max-w-7xl mx-auto pb-6 animate-in fade-in duration-500">
    <div id="tour-decision-header" class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
        <div>
            <h1 class="text-2xl font-black text-bsu-dark uppercase tracking-tight">Resubmission Decision</h1>
            <p class="text-gray-500 text-xs font-medium mt-1">Generate final decision letters for processed revisions</p>
        </div>
    ...
    <div id="tour-decision-list" class="app-card">
        ...
        <div>
            ...
            <template x-for="protocol in filteredData" :key="protocol.id + protocol.version">
                <div id="tour-first-decision-row"
                    ...
                    <div>
                        <span class="text-[10px] font-black uppercase px-2 py-1 rounded border bg-purple-50 text-purple-700 border-purple-200">Synthesis Complete</span>
                        <div class="text-[10px] text-gray-500 font-bold mt-1" x-text="'Processed: ' + protocol.dateSubmitted"></div>
                    </div>
                    <div>
                        <div class="text-[10px] text-gray-700 font-bold" x-text="getMeetingDate(protocol)"></div>
                    </div>
                    <div>
                        <span
            ...
            <div x-show="filteredData.length === 0" class="p-12 text-center text-gray-500 font-bold text-sm">
                No processed resubmissions pending decisions.
            </div>
        </div>
    </div>
        ...
        <div class="modal-box" @click.stop>
            <div class="modal-header">
                <div class="flex items-center gap-3">
                    <h2>Decision Letter Dashboard</h2>
            ...
            <div class="modal-content">
                <div id="tour-decision-sidebar" class="protocol-info-panel">
                    ...
                    <div style="margin-top:30px;">
                        ...
                        <nav class="space-y-1">
                            <button @click="activeView = 'resubmission_form'" :class="activeView === 'resubmission_form' ? 'active' : ''" class="sidebar-nav-item">
                                <span>📋</span> Synthesized Form
                            </button>
                            <button id="tour-nav-compose" @click="activeView = 'decision_letter'" :class="activeView === 'decision_letter' ? 'active' : ''" class="sidebar-nav-item bg-brand-red/10 text-brand-red hover:bg-brand-red/20">
                                <span>✉️</span> Compose Decision
                            </button>
                        </nav>
                    </div>
                    ...
                    <div style="margin-top:30px;">
                        ...
                        <div x-show="!isLoadingDocs" class="space-y-4">
                            ...
                            <template x-if="!isLoadingDocs && loadedDocs.activeBasic.length === 0 && loadedDocs.activeSupp.length === 0">
                                <div class="text-[10px] text-gray-400 italic">No documents found for this revision.</div>
                            </template>
                        </div>
                    </div>
                </div> <div class="form-preview-panel" :class="activeView === 'doc_viewer' ? 'viewer-active' : ''">
                    ...
                    <div x-show="activeView === 'decision_letter'" class="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col h-full m-4 overflow-hidden">
                        ...
                        <div class="p-8 flex-1 overflow-y-auto bg-gray-50/50">
                            <div id="tour-letter-editor" class="max-w-4xl mx-auto bg-white border border-gray-200 rounded-xl p-8 shadow-sm">

                                <div id="tour-decision-dropdown" class="mb-6 p-4 rounded-xl border-2 transition-colors"
                                    :class="{
                                ...
                                <div style="display:flex; flex-direction:column; gap:12px; font-size: 12px; color: #374151;">
                                    ...
                                    <div id="tour-revision-summary" class="mt-8 border border-gray-300 rounded-lg overflow-hidden shadow-sm">
    ...
    Alpine.data('decisionData', (initialData = []) => ({
        ...
        init() {
            this.$watch('decisionStatus', (val) => {
                ...
                if (val === 'approved') {
                    this.letter.paragraph2 = `As a result of the review, we are pleased to inform you that your resubmitted study protocol has been APPROVED. You may proceed with your research.`;
                }
                else if (val === 'minor_revision') {
                    this.letter.paragraph2 = `As a result of the review, your resubmitted study protocol requires MINOR REVISIONS. Please address the recommended revisions and clarifications summarized below before resubmitting your application.`;
                }
                else if (val === 'major_revision') {
                    this.letter.paragraph2 = `As a result of the review, your resubmitted study protocol requires MAJOR REVISIONS. Please thoroughly address the recommended revisions and clarifications summarized below before resubmitting your application.`;
                }
                else if (val === 'rejected') {
                    this.letter.paragraph2 = `As a result of the review, we regret to inform you that your resubmitted study protocol has been DISAPPROVED. The specific findings and reasons leading to this decision are summarized below.`;
                }
                else {
                    this.letter.paragraph2 = `As a result of the review, the final decision for your study is currently pending.`;
                }
            });
        },
        ...
        getAssessmentProcessedDate(protocol) {
            return this.parseDateSafe(
                protocol.assessmentProcessedIssuedAt ||
                protocol.assessment_processed_at ||
                protocol.updated_at ||
                protocol.updatedAt ||
        ...
        getDecisionSchedule(protocol) {
            const now = new Date();
            const baseDate = this.getAssessmentProcessedDate(protocol);
        ...
        getActionRequiredRows() {
            if (!this.selectedProtocol || !this.selectedProtocol.revisionRows) return [];
            return this.selectedProtocol.revisionRows.filter(r => r.action === 'action_required');
        },
        ...
        async openValidate(protocol) {
            ...
            this.letter = {
                ...
                supportDate: today,
                documents: ['', '', '', '', ''], // pre-padded
                paragraph1: `We wish to inform you that the Batangas State University Ethics Committee reviewed your resubmitted study protocol during its regular meeting on ${meetingDate}. Your study has been assigned the code ${protocol.id} which should be used for all communication to the BERC related to this study.`,
            ...
            // Extract numeric version safely
                ...
                if (response.ok) {
                    ...
                    if (data.documents) {
                        Object.keys(data.documents).forEach(type => {
                            ...
                            // Since these are isolated revision documents, they are ALL active for this view
                            files.forEach(file => {
                                ...
                                const obj = {
                                    ...
                                    label: titleLabel,
                                    url: file.url,
                                    isRevised: true, // Inherently true for revisions
                                    desc: displayName
                                };
        ...
        async submitDecision() {
            ...
            try {
                ...
                if (response.ok) {
                    this.showNotification('Success', 'Decision letter sent successfully.');
                    this.selectedProtocol = null;
                    document.body.style.overflow = '';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function runRevisionDecisionTutorial(manual = false, retries = 0) {
            ...
            revisionRows: [
                {
                    id: 'mock_1',
                    item: '1.1',
                    section_and_page: 'p. 12',
                    berc_recommendation: 'Clarify sampling strategy.',
                    researcher_response: 'Updated to purposive sampling.',
                    synthesized_comments: 'Acceptable.',
                    action: 'resolved'
                },
                {
                    id: 'mock_2',
                    item: '4.4',
                    section_and_page: 'p. 3',
                    berc_recommendation: 'Simplify risk language.',
                    researcher_response: 'Revised language used.',
                    synthesized_comments: 'One sentence still too technical. Needs one more minor polish.',
        ...
        const tour = driver({
            ...
            steps: [
                {
                    element: '#tour-decision-header',
                    popover: {
                        title: 'Resubmission Decision',
                        description: 'Welcome to the final stage of the revision workflow. Here, you generate the formal Decision Letter based on the synthesis completed in the previous step.',
                        side: 'bottom'
                    }
                },
                {
                    ...
                    popover: {
                        ...
                        description: 'This list shows all revised protocols that have a completed Secretariat synthesis. They are now waiting for their formal notification letters.',
                        side: 'top'
                    }
                },
                {
                    ...
                    popover: {
                        title: 'Initiate Letter',
                        description: 'Clicking a protocol will open the Decision Dashboard where the letter template is automatically populated with the synthesis results.',
                        side: 'bottom',
                        onNextClick: async () => {
                ...
                },
                {
                    ...
                    popover: {
                        title: 'Compose View',
                        description: 'Switching to this view activates the letter editor.',
                        side: 'right',
                        onNextClick: () => {
